...
`packet-trace-log-grid` is centered on observability work in Scala. The technical target is: Package a Scala local lab for log analysis with transition tables, invalid-transition tests, and documented operating limits.
...
The score is deterministic. The current threshold is 155. Risk has a penalty of 6, latency has a penalty of 3, and weight contributes a bonus of 5.
